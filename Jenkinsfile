node {
   ...
   dir('src') {sh "docker build -t registry.roundtower.io:5000/training9/lab1:${env.BUILD_NUMBER} ."}

   // The next stage uploads the new container to the docker registry
   stage 'Upload Container'

   // Upload the container
   sh "docker push registry.roundtower.io:5000/training9/lab1:${env.BUILD_NUMBER}"
}
